<template>
  <div>
    <h1>座位预约信息</h1>
    <div v-for="item in getPagedData()" :key="'current-' + item.bookingID" class="card">
      <div class="container">
        <div class="left">
    ...
    <div v-if="bookinglist.length === 0" class="empty-message">
      暂无预约信息
    </div>
    
    <!-- 分页组件 -->
    <div v-if="bookinglist.length > 0" class="pagination-container" style="margin-top: 20px; text-align: right;">
      <el-pagination
...
<script>
import { bookingmessage, deletebooking } from '@/api/dept';
...
export default {
  ...
  methods: {
    async getbookingmsg() {
      const res = await bookingmessage(this.userPhone,'N')
      console.log('当前预约数据:', res)
    ...
    getPagedData() {
      ...
      return this.bookinglist.slice(start, end)
    },
    /**
     * 处理页码变化
     */
    ...
    async cancelBooking(bookingID) {
      const confirmCancel = confirm('是否取消预约？')
      if (confirmCancel) {
        try {
